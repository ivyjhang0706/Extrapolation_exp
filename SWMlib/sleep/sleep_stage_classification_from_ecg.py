...
def sleep_stage_classification_xgboost_(ridxs, gender, age, model_d_p):
    ...
    for j in range(ridxs_30sec_len):
        ...
        ##ridxs = [ridx - stage_index_s*15000 for ridxs_i in ridxs_30sec[stage_index_s:stage_index_e] for ridx in ridxs_i]
    ...
    new_feature_dict['TIME'] = results['TIME']
    new_feature_dict['GENDER'] = results['GENDER']
    new_feature_dict['AGE'] = results['AGE']
    ...
    for key in results.keys():
        ...
        if 'HRV' in key:
            ...
            now_hrv_list = np.array(results[key])
    ...
    new_feature_dict = pd.DataFrame(new_feature_dict).replace([np.inf, -np.inf], np.nan).dropna(axis=1, how='all')
    ...
    X_data = new_feature_dict.iloc[:, 1:]
    y_data = new_feature_dict.iloc[:, 1]
...
if __name__ == "__main__":
    ...
    shhs_ridx_d_p = r'C:\Users\SWM_Jared\Desktop\Project\Sleep Staging\2-RIDX-NK'
    model_d_p = r'C:\Users\SWM_Jared\Desktop\Project\Sleep Staging\model_doc'
    shhs_file = 'shhs1-200001'

    f_info = open(os.path.join(shhs_ridx_d_p, '{}_ridx.json'.format(shhs_file)))
    f_data = json.load(f_info)
    f_info.close()

    ridxs = np.array(f_data["ridx"])

    gender = 1
    age = 55

    pred_list = sleep_stage_classification_xgboost_(ridxs, gender, age, model_d_p)
